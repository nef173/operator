...
export default function DashboardPage() {
  ...
  // Two faces, each hiding any app the user isn't granted. Frontend = the build apps;
  // the backend face renders as GROUPED section lines (Finance / Backend / Operation /
  // App Settings) driven by each app's `group` field in the registry.
  const frontendApps = OPERATOR_APPS.filter((a) => !restricted.includes(a.id));
  const backendApps = BACKEND_APPS.filter((a) => !restricted.includes(a.id));
  const backendSections = BACKEND_GROUPS.map((g) => ({
    ...g,
    apps: backendApps.filter((a) => a.group === g.group),
  })).filter((g) => g.apps.length > 0);
  // Operation-wide P&L KPIs — real, from Finance. Date scope + store scope are operator-selectable;
  // defaults are ALWAYS today (1-day window) and all stores (aggregate).
  const [win, setWin] = useState<WindowKey>("today");
  const [storeKey, setStoreKey] = useState("");
  // Reporting timezone the Today/Yesterday windows anchor to — the same key the Finance app writes
  // (BKK/CET/NY chips). Lazy read (SSR-safe); default CET.
  const [reportTz] = useState<string>(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("fin_reporting_tz") || "Europe/Berlin"
      : "Europe/Berlin",
  );
  const health = useApi(() => api.finStoreHealth(30, reportTz), [reportTz]);
      ...
      <div className="relative z-10 flex min-h-full flex-col">
        ...
        <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-5 py-7 sm:px-8 sm:py-9">
          <div className="my-auto w-full">
            ...
            {/* App cards — split by face. Frontend is the build half; Backend is the
                operation backend (the run half + control plane). */}
            {frontendApps.length ? (
              <>
                <FaceHeading title="Frontend" />
            ...
            {backendSections.map((section) => (
              <div key={section.group}>
                <FaceHeading title={section.title} />
                <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
...
function FaceHeading({ title }: { title: string }) {
  return (
    <div className="mt-10 flex items-baseline gap-3 sm:mt-12">
      <span
        className="text-[11px] font-semibold uppercase sm:text-[12px]"
        style={{ letterSpacing: ".22em", color: "var(--accent)" }}
      >
        {title}
      </span>
...
function HeaderButton({
  ...
  return (
    <button
      ...
      title={label}
      className="flex h-11 w-11 items-center justify-center rounded-xl border border-[var(--border)] text-[var(--muted)] transition-colors hover:bg-[var(--surface-2)] hover:text-[var(--text)]"
    >
